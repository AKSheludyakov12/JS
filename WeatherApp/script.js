...
const link = "http://api.weatherapi.com/v1/forecast.json?key=f640eab122354f39be692227232108&days=5&lang=ru"
      ...
      console.log(nextDays)
        ...
        mappedNextDays = nextDays.map(day=>{
            return {
                ...
                horlyForecast:(day.hour.map(hour=>{
                    return {time: formatTime(hour.time),
                        temp:Math.round((hour.temp_c)),
                        chanceOfRain:(hour.chance_of_rain),
                        description: (hour.condition.text),}
                }))
        ...
        nextFiveHours = forecastEveryHour.slice(currentHour, currentHour + 6);
            if(nextFiveHours.length < 6){
                for(let i of tommorowHorlyForecast)
                nextFiveHours.push(i)
            }
...
const renderHourlyForecast =(nextFiveHours) =>{
    ...
    <div class="hour">${nextFiveHours[i].time}</div>
        <div class="forecsat-icon"><img src="img/icon/day/${getImage(nextFiveHours[i].description)}" alt=""></div>
...
const renderTomorrowForecast =(tommorowHorlyForecast, tommorow) =>{
    ...
    <div class="hour">${tommorowHorlyForecast[i].time}</div>
        <div class="forecsat-icon"><img src="img/icon/day/${getImage(tommorowHorlyForecast[i].description)}" alt=""></div>
...
const renderNextDays = (mappedNextDays) => {
    ...
    <div class="forecsat-icon"><img src="img/icon/day/${getImage(mappedNextDays[i].description)}" alt=""></div>
    </li>`
    }
    return markup
}
...
const markup = () =>{  
    const { properties, maxtemp, mintemp, temp, city, feelsLike, discription, condition, formattedTime, localTime} = store
    return `
    <head>
    <div class="top">
    <div class="city">
...
<div class="temp-info">
    <div class="degree">
        ...
        <div class="feels-like"><p>Ощущается как ${Math.round(store.feelsLike)} </p></div>
    </div>
    <div class="desc">
        <div class="desc-icon"><img src="img/icon/day/${getImage(condition)}" alt=""></div>
